...
describe('tutorial: storage s3_multipart_uploads_parts table access', () => {
  ...
  it('should verify authenticated access to s3_multipart_uploads_parts based on rls', async () => {
    if (!tableExists) {
      return;
    }
    db.setContext({ role: 'service_role' });
    const rlsStatus = await db.any(
      `SELECT c.relrowsecurity 
       ...
       WHERE n.nspname = 'storage' AND c.relname = 's3_multipart_uploads_parts'`
    );
    expect(Array.isArray(rlsStatus)).toBe(true);
    
    db.setContext({ role: 'authenticated', 'request.jwt.claim.sub': testUserId });
    const result = await db.any(`SELECT * FROM storage.s3_multipart_uploads_parts LIMIT 1`);
  ...
  it('should verify anon access to s3_multipart_uploads_parts based on rls', async () => {
    if (!tableExists) {
      return;
    }
    db.setContext({ role: 'service_role' });
